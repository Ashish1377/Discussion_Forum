...
const Askquestion = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response && response[0] && response[0][0]) {
        ...
        if (label_0 === "LABEL_0") {
          toast.error("Your question seems to be spam", { duration: 2000 });
          return;
        }
      } else {
        toast.error("Unexpected response format from spam detection service");
  ...
  async function query(data) {
    const response = await fetch(
      ...
      {
        headers: {
          Authorization: "Bearer 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",
        },
        method: "POST",
